module inertial_integrator(clk,rst_n,strt_cal,cal_done,vld,rdy,yaw_rt,
  ...
  reg vld_ff1,vld_ff2,vld_ff3;		// pipe vld to keep synch (ff3 tracks yaw_corr)
  reg signed [26:0] yaw_corr;			// pre-summed (yaw_scaled+yaw_fusion), breaks integrator carry chain
...
*/	  
  ...
  //// now pipeline vld 3X to form rdy (extra stage matches yaw_corr pipe) ////
  always_ff @(posedge clk, negedge rst_n)
    if (!rst_n) begin
	  vld_ff1 <= 1'b0;
	  vld_ff2 <= 1'b0;
  ...
  ///////////////////////////////////////////////////////////////
  // Pre-register the (yaw_scaled + yaw_fusion) sum so that   //
  // the integrator below collapses into a single 27-bit add //
  // instead of two cascaded ripple adders (timing fix).    //
  ///////////////////////////////////////////////////////////
  always_ff @(posedge clk)
    if (vld_ff2) begin
	  if (FAST_SIM)
	    yaw_corr <= {{6{yaw_scaled[18]}},yaw_scaled,2'b00} +
	                {{7{yaw_fusion[18]}},yaw_fusion,1'b0};
	  else
	    yaw_corr <= {{8{yaw_scaled[18]}},yaw_scaled} +
  ...
  always @(posedge clk, negedge rst_n)
    if (!rst_n) begin
	  ...
	  yaw_int <= 0;
	end else if ((vld_ff3) && ((!compensate_offset) || (moving))) begin
	  /////////////////////////////////////////////////////////////////
