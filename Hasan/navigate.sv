module navigate(clk,rst_n,strt_hdng,strt_mv,stp_lft,stp_rght,mv_cmplt,hdng_rdy,moving,
  ...
  typedef enum {IDLE, START_MV, MOVING, HEADING} state_t;
	  ...
	  frwrd_spd <= 11'h000;
	    ...
	    frwrd_spd <= ((dec_frwrd_fast) && (frwrd_spd>{2'h0,frwrd_inc,3'b000})) ? frwrd_spd - {2'h0,frwrd_inc,3'b000} : // 8x accel rate
                   (dec_frwrd_fast) ? 11'h000 :	  // if non zero but smaller than dec amnt set to zero.
	                 (frwrd_spd>{4'h0,frwrd_inc,1'b0}) ? frwrd_spd - {4'h0,frwrd_inc,1'b0} : 11'h000;

  //<< Your implementation of ancillary circuits and SM >>	

  //rising edge detect for openings
  always_ff @(posedge clk, negedge rst_n) begin
    if(!rst_n) begin
      lft_opn_rise <= 1'b0;
      rght_opn_rise <= 1'b0;
    end else begin
      lft_opn_rise <= ((lft_opn & stp_lft) & ~lft_opn_rise);
      rght_opn_rise <= ((rght_opn & stp_rght) & ~rght_opn_rise);
    end
  end
  ...
  assign en_fusion = (frwrd_spd>=MAX_FRWRD>>1) ? 1'b1 : 1'b0;	//only enable fusion when moving at 1/2 max speed
  ...
  always_comb begin
    ...
    case(state)
      ...
      MOVING: begin
        inc_frwrd = 1'b1;
        moving = 1'b1;
        if (~frwrd_opn || lft_opn_rise || rght_opn_rise)
          if (~frwrd_opn)	begin //if we are stopping for an opening use fast deceleration to stop at the opening.  Otherwise just decelerate at normal rate.
            dec_frwrd_fast = 1'b1;
          end else begin  
            dec_frwrd = 1'b1;
          end
          if(frwrd_spd == 11'h000) begin
            mv_cmplt = 1'b1;
            next_state = IDLE;	//go back to idle and wait for next command
          end
        else
          next_state = MOVING;
      end
      ...
      HEADING: begin
        ...
        if (at_hdng) begin
          ...
          next_state = HEADING;	//stay here until heading complete
      end
